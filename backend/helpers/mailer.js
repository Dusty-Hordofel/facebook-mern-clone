...
{
  /*import nodemailer from 'nodemailer'; //nodemailer is a library that allows us to send emails
import { google } from 'googleapis'; //
import dotenv from 'dotenv';
...
export const sendVerificationEmail = (email, name, url) => {
  ...
  const accessToken = auth.getAccessToken(); //getAccessToken is used to get the access token
  ...
  }); //createTransport is used to create a transporter for sending emails
  const mailOptions = {
    from: EMAIL,
    to: email,
    subject: 'Facebook email verification',
